...
static auto CreateFile( const char* path,const std::vector<int> shape, const std::vector<int> chunksize){
	...
	if(level==0)
	 json_spec = GetJsonSpec(path,shape,shape);
	else
	 json_spec = GetJsonSpec2(path,level,shape,shape);
...
while (iss >> ch) {
	 if (std::isdigit(ch) || (ch == '-' && std::isdigit(iss.peek()))) {
                iss.putback(ch);
            iss >> value;
                parsed_data.push_back(static_cast<int32_t>(value));
	     }
    }
std::cout <<"parsing are fine:";
std::vector<int32_t> data_to_use(parsed_data.begin() + 4, parsed_data.end());
...
for (Index i = 0; i < rows; ++i) {
	//for (Index j = 0; j < cols; ++j) {
	    ...
	    array(i) = data_to_use[i];
	
}
*/
	const Index cols = shape[1];
	auto array=tensorstore::AllocateArray<int32_t>({rows,cols});
	for (Index i = 0; i < rows; ++i) {
	for (Index j = 0; j < cols; ++j) {
		//std::cout << "writing: " << i*cols+j<<" i: "<<i<<" j: "<<j<<std::endl;
...
const Index cols = shape[1];	
const Index depth = shape[2];
auto array=tensorstore::AllocateArray<int32_t>({rows,cols,depth});
	for (Index i = 0; i < rows; ++i) {
    for (Index j = 0; j < cols; ++j) {
        for (Index k = 0; k < depth; ++k) {
            //std::cout << "writing: " << (i*cols*depth + j*depth + k) << " i: " << i << " j: " << j << " k: " << k << std::endl;
            array(i,j,k) = data_to_use[i*cols*depth + j*depth + k];
        }
    }
}
