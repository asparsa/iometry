...
::nlohmann::json GetJsonSpec(std::vector<int> shape) {
  return {
      {"driver", "zarr3"},
      {"kvstore", {{"driver", "file"}, {"path", "/scratch/bbsn/asalimiparsa/testo/"},{"file_io_concurrency",{{"limit",1}}}}},
      {"metadata",
       {{"data_type", "float64"},
        {"shape", shape},
        {"chunk_grid",
...
static auto CreateFile( std::vector<int> shape){
	...
	json_spec = GetJsonSpec("sd",shape,chunksize,1);
...
sprintf(fileName, "/scratch/bbsn/asalimiparsa/zarr_%03d",dumpn);
