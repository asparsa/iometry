...
std::uniform_int_distribution<> distrib(0.0, 1.0);
...
*/
::nlohmann::json GetJsonSpec(int th) {
  return {
      {"driver", "zarr3"},
      {"kvstore", {{"driver", "file"}, {"path", "testo/"},{"file_io_concurrency",{{"limit",th}}}}},
      {"metadata",
       {{"data_type", "float64"},
        {"shape", {16384,16384}},
        {"chunk_grid",
         {{"name", "regular"},
          {"configuration", {{"chunk_shape", {16384,16384}}}}}}}},
  };
}
::nlohmann::json GetJsonSpec2(int th,int level) {
  return {
      {"driver", "zarr3"},
      {"kvstore", {
          {"driver", "file"},
      ...
      {"metadata", {
          ...
          {"codecs", {{
              {"name", "gzip"},
              {"configuration", {
                  {"level", level}  // Compression level from 1 (fastest) to 9 (smallest)
              }}
          }}}
      }}
  };
              ...
              {"configuration", {
                  ...
                  {"chunk_codec_spec", {
                      {"codecs", {{
                          {"name", "gzip"},
                          {"configuration", {
                              {"level", level}  // Compression level (1-9), adjust as needed
                          }}
                      }}}
                  }}
              }}
...
::nlohmann::json GetJsonSpec(const char* path, const int level, const std::vector<int> shape, const std::vector<int> chunksize,const int numthread) {
  return ::nlohmann::json{
        {"driver", "zarr3"},
        {"kvstore",
         {
...
*/
::nlohmann::json GetJsonSpec3( int level, int numthread) {
    return ::nlohmann::json{
        {"driver", "zarr3"},
        {"kvstore",
         {
             {"driver", "file"},
             {"path","testo/"},  // Use the provided path parameter
        ...
        {"metadata",
         {
             {"zarr_format", 3},
             {"data_type", "float64"},
             {"shape", {16384,16384}},  // Use the provided shape parameter
             {"codecs", {
                 {
                     {"name", "gzip"},
                     {"configuration", {
...
auto store_result = tensorstore::Open(GetJsonSpec3(level,num_th), context, tensorstore::OpenMode::create, tensorstore::ReadWriteMode::read_write);
return store_result;
}
...
std::vector<int> shape={16384,16384};
